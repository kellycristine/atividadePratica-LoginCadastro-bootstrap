    ...
    rowRecados.innerHTML += `
        <div class="col-12 col-md-3">
            ...
            <div class="card-body">
                ...
                <button class="btn btn-danger" data-id="${item.id}" onclick="excluirRecado(this)">Excluir</button>
                </div>
            </div>
            </div>
        </div>
...
// Evento para criar um novo recado
document.getElementById("btn-criar-recado").addEventListener("click", async () => {
    const titulo = prompt("Digite o título do recado:");
    const descricao = prompt("Digite a descrição do recado:");
  
    if (titulo && descricao) {
      const novoRecado = { titulo, descricao };
      try {
        const response = await axios.post("http://localhost:5555/recados", novoRecado);
        buscarRecados();
      } catch (error) {
        console.log(error);
      }
